  ...
  await Promise.all(studentDataList.map(async student => {
    ...
    if (student.block1) {
      const block1Teacher = await context.query.User.findMany({
        where: { email: {equals: student.block1} }, query: graphql`
      id
    email
    ` });
    ...
    if (student.block2) {
      const block2Teacher = await context.query.User.findMany({
        where: { email: {equals: student.block2} }, query: graphql`
      id
    email
    ` });
    ...
    if (student.block3) {
      const block3Teacher = await context.query.User.findMany({
        where: { email: {equals: student.block3} }, query: graphql`
      id
    email
    ` });
    ...
    if (student.block4) {
      const block4Teacher = await context.query.User.findMany({
        where: { email: {equals: student.block4} }, query: graphql`
      id
    email
    ` });
    ...
    if (student.block5) {
      const block5Teacher = await context.query.User.findMany({
        where: { email: {equals: student.block5} }, query: graphql`
      id
    email
    ` });
