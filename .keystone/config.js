...
var QueryGeneratorService = class _QueryGeneratorService {
...
${wasTruncated ? `19. CRITICAL - THE RESULTS ARE INCOMPLETE. They were cut to fit, and the
...
${isEmpty ? "IMPORTANT: Since no data was found, you MUST provide a suggested_followup with alternative search strategies." : ""}`;
   ...
   */
  WEEK_SCOPED = /\b(?:this|last|past)\s+week\b/i;
  CARD_SUBJECT = /\b(?:card|cards|pbis)\b/i;
  /**
   ...
   */
  static CONTENTLESS_TOKENS = /* @__PURE__ */ new Set([
    "hi",
    "hii",
    "hiya",
    "hello",
   ...
   * nothing to query.
   */
  hasNoAnswerableContent(question) {
    const tokens = question.toLowerCase().replace(/[^a-z0-9\s]/g, " ").split(/\s+/).filter(Boolean);
    if (tokens.length === 0) return true;
    return tokens.every(
      (t) => _QueryGeneratorService.CONTENTLESS_TOKENS.has(t)
    );
  }
  async seedCollectionDates(question) {
    const scoped = this.COLLECTION_SCOPED.test(question) || this.WEEK_SCOPED.test(question) && this.CARD_SUBJECT.test(question);
  ...
  async processQuery(question, model, userId, userName) {
    ...
    if (this.hasNoAnswerableContent(question)) {
      ...
      return {
        ...
        data: null,
        explanation: "I did not find a question in that. Ask me about the data in the school system and I will look it up - for example: who gave the most PBIS cards in the last collection, which students have open callbacks, or how many cards a particular student has received.",
        iterations: 0
      };
    }
    ...
    let allData = [];
    const priorSteps = [];
    const executedQueries = /* @__PURE__ */ new Set();
